    ...
    int capacity)
{
    ...
    {
        std::cerr << "Memory allocation failed: " << error.what() << endl;
    }
...
string XArrayList<T>::toString(string (*item2str)(T &))
{
    ...
    // TODO
    stringstream strArray;
    strArray << "[";
    for (int i = 0; i < count; i++)
    {
        if (item2str != nullptr)
        {
            strArray << item2str(data[i]);
        }
        else
        {
            strArray << data[i];
...
void XArrayList<T>::ensureCapacity(int index)
{
    ...
    if (index >= capacity || count == capacity) {
        int newCapacity = (index + 1 <= capacity * 2) ? capacity * 2 : index + 1;
